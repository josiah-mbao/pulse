document.addEventListener('DOMContentLoaded', () => {
  ...
  tabButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      tabButtons.forEach(b => {
        b.classList.remove('border-emerald-500', 'text-emerald-400', 'bg-zinc-800/50');
      ...
      btn.classList.add('border-emerald-500', 'text-emerald-400', 'bg-zinc-800/50');
  ...
  if (terminalLogs) {
    ...
    function addTerminalEvent() {
      ...
      const eventClass = isExec ? 'text-emerald-400 font-semibold' : 'text-rose-400 font-semibold';
      ...
      line.innerHTML = `
        ...
        <span class="${eventClass} px-1.5 py-0.5 rounded bg-zinc-900 border ${isExec ? 'border-emerald-500/30' : 'border-rose-500/30'}">${eventType}</span>
        <span class="text-zinc-400">pid=<span class="text-zinc-200">${proc.pid + count}</span></span>
        <span class="text-zinc-300 truncate">comm=<span class="text-emerald-300">"${proc.comm}"</span></span>
      `;
